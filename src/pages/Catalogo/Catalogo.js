...
function Catalogo() {
  ...
  const aplicarFiltros = async () => {
    ...
    if (filtros.classificacaoIdade) filtroQuery += `classificacaoIdade=${filtros.classificacaoIdade}&`;
    if (filtros.lotacaoMaxima) filtroQuery += `lotacaoMaxima=${filtros.lotacaoMaxima}&`;
    if (filtros.nomeEvento) filtroQuery += `nomeEvento=${filtros.nomeEvento}&`;
    if (filtros.statusEvento) filtroQuery += `statusEvento=${filtros.statusEvento}&`;

    try {
      const response = await fetchPublicData(`evento/filtro?${filtroQuery.slice(0, -1)}`);
  ...
  return (
      <Container fluid className="p-5">
        <Row>
          <Col md={3} lg={3}>
            <FiltroEventos filtros={filtros} onChange={setFiltros} onApply={aplicarFiltros} onClear={limparFiltros} />
          </Col>
          <Col md={9} lg={9}>
            {eventos.length > 0 ? (
              <Row>
